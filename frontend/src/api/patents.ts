 ...
 */
export const normalizePatentResponse = (
  response: any,
  apiSource: ApiSource
) => {
  ...
  if (
    ...
    (typeof data.search_metadata === "object" ||
  ...
  if (apiSource === "unified") {
    ...
    if (data.organic_results && Array.isArray(data.organic_results)) {
      ...
      const normalized = {
        ...
        details: {
          assignee_current:
            data.assignee_current || (data.assignee ? [data.assignee] : []),
...
export const patentApi = {
  ...
  searchPatentsUnified: async (
    patentNumber: string
  ): Promise<UnifiedPatentResponse> => {
    // Keep direct axios for third-party APIs that don't need auth
    const response = await axios.get(
      `https://api.unifiedpatents.com/patents/${patentNumber}?with_cases=true`
    );
    return response.data;
  },
  ...
  searchMultiplePatentsUnified: async (patentNumbers: string[]) => {
    const payload = {
      query: {
        bool: {
          must: [
    ...
    const response = await axios.post(
      "https://api.unifiedpatents.com/patents/v6/_search",
      payload
    );
  ...
  getFullLanguage: async (
    patentNumber: string
  ): Promise<FullLanguageResponse> => {
    // Keep direct axios for third-party APIs that don't need auth
    const response = await axios.get(
      `https://api.unifiedpatents.com/patents/${patentNumber}/full-language`
    );
  ...
  getFigures: async (patentNumber: string) => {
    try {
      ...
      const response = await axios.get(
        `https://api.unifiedpatents.com/patents/${patentNumber}/figures`
      );
      return response.data;
    } catch (unifiedError) {
      ...
      try {
        ...
        throw new Error(
          "Failed to fetch patent figures from any available source"
        );
      }
    }
  },
  ...
  searchPatentsForValidation: async (ids: string[]) => {
    ...
    const foundPatentIds = new Set(
      foundPatents.map((p: PatentResult) => p.patent_id)
    );
  ...
  transformPatentIds: async (ids: string[]) => {
    const response = await axios.post(
      "https://api.unifiedpatents.com/helpers/transform-publication-numbers",
      {
        publications: ids,
      }
    );
  ...
  filterPatentsByFamily: async (
    ...
    const response = await axiosInstance.post(
      `${API_URL}/patents/filter-by-family`,
      {
        patents,
